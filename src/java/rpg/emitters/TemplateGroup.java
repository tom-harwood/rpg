...
{
    public TemplateGroup(String templateDirName, String templateGroupName)
    {
        if (templateDirName != null) {
            templateGroupName = templateDirName + "/" + templateGroupName;
        }
        this.templates = new STGroupFile(templateGroupName);

        setDefaultAttribute("timestamp", new java.util.Date());
        setDefaultAttribute("version", "Potzrebie MK I");
    }
    ...
    public ST getTemplate(String name, Object ... attrValue)
    {
        ...
        if (result == null) {
            throw new IllegalStateException(String.format("Internal error: cannot find template " + name + "(%s)", attrValue));
        }
